...
import { EntityArrayResponseType, ParkingSpotService } from '../service/parking-spot.service';
...
export class ParkingSpotComponent implements OnInit {
  ...
  trackId = (_index: number, item: IParkingSpot): number => this.parkingSpotService.getParkingSpotIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.parkingSpots = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IParkingSpot[] | null): IParkingSpot[] {
    return data ?? [];
  }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.parkingSpotService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
